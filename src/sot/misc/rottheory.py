...
def modmax(Theta, zeta, inc, Thetaeq):
    fac1 = (
        -np.cos(zeta)
        + np.cos(inc) * np.sin(zeta) * np.sin(Theta - Thetaeq)
        - np.cos(zeta) * np.cos(Theta) * np.sin(inc)
    )
    fac2 = (
        np.cos(Theta - Thetaeq) ** 2
        + 2 * np.cos(Theta - Thetaeq) * np.cos(Thetaeq) * np.sin(inc)
        + np.cos(Thetaeq) ** 2 * np.sin(inc) ** 2
        + (
            np.cos(inc) * np.sin(zeta)
            - np.cos(zeta) * np.sin(Theta - Thetaeq)
            + np.cos(zeta) * np.sin(inc) * np.sin(Thetaeq)
        )
        ** 2
    )
    return fac1 / fac2
...
if __name__ == "__main__":
    ...
    plt.imshow(
        arr, cmap="coolwarm", vmin=-1.0, vmax=1.0, extent=[0, 2 * np.pi, np.pi, 0]
    )
