...
def log_prior(theta):
    p_zeta,p_Thetaeq,p_gamma,p_alpha,p_tau=theta
    if 0.0 <= p_zeta <= np.pi and 0.0 <= p_Thetaeq <= 2*np.pi and 0.01 <= p_gamma <= np.pi/2.0 and 1.e-4 <= p_alpha <= 1.e4 and 1.e-4 <= p_tau <= 1.e4:
        return np.log(np.sin(p_zeta)/p_alpha/p_gamma/p_tau)
    return -np.inf
...
pos = np.array([zeta,Thetaeq,gam0,alpha0,tau0])+ 1e-4 * np.random.randn(32, 5)
...
#print(samples)
labels=["zeta","Thetaeq","gamma","alpha","tau"]
